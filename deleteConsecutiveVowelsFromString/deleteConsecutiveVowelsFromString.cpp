#include "deleteConsecutiveVowelsFromStringDef.hpp"
...
void stringWorkFlow::deleteConsecutiveVowelsFromString (oneDimensionalArrayType<char> dataWorkFlow) {

  if (__validations__.isZero(dataWorkFlow.oneDimensionalArray)) throw systemException ("Unable to process length as zero");
  if (__validations__.isNegative(dataWorkFlow.oneDimensionalArray)) throw systemException ("Unable to process negative length");

  for (int iterator = dataWorkFlow.startPoint; iterator < strlen(dataWorkFlow.oneDimensionalArray) - 1; iterator++)
    if (isVowel (dataWorkFlow.oneDimensionalArray[iterator])) 
      if (isVowel (dataWorkFlow.oneDimensionalArray[iterator + 1])) {
      strcpy (dataWorkFlow.oneDimensionalArray + iterator + 1, dataWorkFlow.oneDimensionalArray + iterator + 2);
      iterator -= 1;
    }
}
...
int main (int argc, char const *argv[]) {
  ...
  workflow.deleteConsecutiveVowelsFromString (string);
