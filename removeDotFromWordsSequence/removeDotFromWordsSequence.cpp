#include "removeDotFromWordsSequenceDef.hpp"
...
void removerProcessor::outputNonDotElements (char * CharPointer) {

  for (int iterator = 0; iterator < sizeof(CharPointer); iterator++)
    if (*(CharPointer + iterator) == ',')
      break;
    else
      std::cout << *(CharPointer + iterator) << " ";
}

void removerProcessor::RemoveDotFromWordsSequence (oneDimensionalArrayType<char> ODAObject) {
  ...
  while (PointerChar) {
    ...
    if (strchr(PointerChar, ','))
      outputNonDotElements (PointerChar);
    else 
...
int main (int argc, char const * argv[]) {
  ...
  __processor__.RemoveDotFromWordsSequence (charArray);
